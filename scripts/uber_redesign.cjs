...
try {
  ...
  if (!code.includes(breakPoint)) {
    console.error("No se encontró el breakpoint 'if (loading)'");
    process.exit(1);
  }
    ...
    return (
      <div className="h-[100dvh] w-full bg-[#f3f4f6] dark:bg-background relative overflow-hidden flex flex-col font-sans">
        ...
        <div className="absolute bottom-0 w-full h-[40vh] bg-white dark:bg-card rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-8">
           <div className="w-16 h-1.5 bg-gray-300 dark:bg-gray-700 rounded-full mx-auto mb-8 animate-pulse"></div>
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f3f4f6] dark:bg-background flex-col gap-4 font-sans px-6 text-center">
        ...
        <Button onClick={() => navigate('/')} className="mt-4 rounded-2xl h-14 px-8 bg-[#FF5A1F] hover:bg-[#e04a17] text-white font-bold text-lg">Volver al inicio</Button>
      </div>
    );
  }
  ...
  return (
    <div className="h-[100dvh] w-full bg-white dark:bg-background relative overflow-hidden font-sans">
      ...
      <div className="absolute top-0 left-0 w-full h-[55vh] sm:h-[60vh] z-0 bg-muted">
      ...
      {/* Bottom Sheet (Draggable-style panel) Uber Style */}
      <div className="absolute bottom-0 left-0 w-full h-[55vh] sm:h-[45vh] bg-white dark:bg-card rounded-t-[2.5rem] shadow-[0_-15px_40px_rgba(0,0,0,0.15)] z-20 flex flex-col">
        {/* Drag Handle */}
          ...
          {status === 'cerrada' && !encuestaEnviada && localStorage.getItem(\`encuesta_completada_\${token}\`) !== 'true' ? (
            <div className="text-center py-8">
              <div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6">
                <XCircle className="w-10 h-10 text-[#FF5A1F]" />
              </div>
              ...
              <Button 
                ...
                className="w-full bg-[#FF5A1F] hover:bg-[#e04a17] text-white font-bold rounded-2xl h-14 shadow-lg text-lg"
              >
            ...
            <>
              ...
              <div className="flex justify-between items-end mb-6">
                <div>
                  <p className="text-[11px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider mb-1">Tracking Nº</p>
                  <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">VTEXT - {data.idoperacion || token}</h2>
                </div>
              ...
              {tecnico && status !== 'finalizada' && (
                <div className="flex items-center gap-4 bg-gray-50 dark:bg-muted/40 p-4 rounded-2xl mb-8 border border-gray-100 dark:border-border/50">
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-gray-900 dark:text-white text-[15px] leading-tight mb-1">
              ...
              {/* Vertical Timeline - FIGMA REPLICA */}
              <div className="relative pl-[22px] border-l-2 border-gray-200 dark:border-gray-800 ml-4 space-y-8 mb-10">
                {steps.map((step, i) => {
                  ...
                  return (
                    <div key={step.id} className="relative">
                      {/* Timeline Dot */}
                      <div className={\`absolute -left-[31px] w-4 h-4 rounded-full border-[3px] border-white dark:border-card \${isPast ? 'bg-[#FF5A1F]' : 'bg-gray-300 dark:bg-gray-700'}\`}>
                        {isCurrent && (
                          <div className="absolute -inset-1.5 rounded-full border border-[#FF5A1F] animate-ping opacity-50"></div>
                        )}
                      </div>
              ...
              {status !== 'finalizada' && status !== 'cerrada' && (
                <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-border/50">
                  <button 
                    ...
                    className="flex-1 bg-gray-900 text-white dark:bg-white dark:text-black h-12 rounded-full text-[13px] font-bold active:scale-95 transition-transform"
                  >
                    Reprogramar solicitud
                  </button>
                  <button 
                    onClick={() => setIsCancelModalOpen(true)}
                    className="flex-1 bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-400 h-12 rounded-full text-[13px] font-bold active:scale-95 transition-transform"
                  >
                    Cancelar visita
                  </button>
                </div>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {isSubmittingReprogram ? "Enviando..." : "Confirmar"}
                  </button>
                </div>
              </div>
            </motion.div>
