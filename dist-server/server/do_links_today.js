...
async function main() {
    try {
        ...
        const [rows] = await pool.query('SELECT idoperacion FROM OPERACION WHERE DATE(fecha_programacion) = CURDATE() LIMIT 10');
        if (rows.length === 0) {
            ...
            const [fallback] = await pool.query('SELECT idoperacion FROM OPERACION ORDER BY fecha_programacion DESC LIMIT 2');
