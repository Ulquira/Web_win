...
export function useInstalacion(token) {
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(false);
    const previousStatus = useRef(null);
    const triggerNotification = (newStatus, tecnicoData, logoImg) => {
        let title = "¡Actualización de tu servicio!";
        let body = "";
        switch (newStatus) {
            case 'asignado':
                body = `El técnico ${tecnicoData?.nombre || ''} ha sido asignado a tu instalación.`;
                break;
            case 'en_camino':
                body = "Tu técnico ya está en camino a tu domicilio. Revisa el mapa.";
                break;
            case 'en_proceso':
                body = "La instalación está en proceso en este momento.";
                break;
            case 'finalizada':
                body = "Instalación completada. Por favor evalúa nuestro servicio.";
                break;
            default: return;
        }
